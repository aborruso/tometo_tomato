...
A seguira una tabellina di riepilogo, sull'esempio del nome del [Comune di `Forza·d'Agrò`](https://www.wikiwand.com/en/articles/Forza_d'Agr%C3%B2), scritto però in modo errato `Forza··D'Agro·`:
...
| Passaggio | Distanza | Confronto | Query SQL Eseguita |
| :--- | :--- | :--- | :--- |
| Dati Grezzi | 5 | `Forza··D'Agro·`; `Forza·d'Agrò` | `SELECT levenshtein('Forza  D''Agro ', 'Forza·d''Agrò') AS distance;` |
| + `LOWER()`, ovvero case insentive | 4 | `forza··d'agro·`; `forza·d'agrò` | `SELECT levenshtein(LOWER('Forza  D''Agro '), LOWER('Forza·d''Agrò')) AS distance;` |
| + Normalizzazione spazi | 2 | `forza·d'agro`; `forza·d'agrò` | `SELECT levenshtein(regexp_replace(trim(LOWER('Forza  D''Agro ')), '\s+', ' '), regexp_replace(replace(trim(LOWER('Forza·d''Agrò')), '·', ' '), '\s+', ' ')) AS distance;` |
| + `strip_accents()`, convertendo `ò` in `o` | 0 | `forza·d'agro`; `forza·d'agro` | `SELECT levenshtein(strip_accents(regexp_replace(trim(LOWER('Forza  D''Agro ')), '\s+', ' ')), strip_accents(regexp_replace(replace(trim(LOWER('Forza·d''Agrò')), '·', ' '), '\s+', ' '))) AS distance;` |
: Riduzione della distanza con passaggi successivi di normalizzazione {.striped .small tbl-colwidths="[20,20,20,40]"}
